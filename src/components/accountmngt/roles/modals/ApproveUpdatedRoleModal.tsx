...
import {createRole, updateRole} from "@/lib/services/accountmngt/roleService";
import Spinner from "@/components/shared/icons/Spinner";

export default function ApproveUpdatedRoleModal({stagingRecord, isOpen, onClose}: {
    ...
    const {user} = useAuth();
    const [updateRoleRequest, setUpdateRoleRequest] = useState<UpdateRoleRequest>({} as UpdateRoleRequest);
    ...
    useEffect(() => {
        ...
        const roleRequest: UpdateRoleRequest = {
            roleId: jsonData.roleId,
            description: jsonData.description,
            name: jsonData.name
        };
        setUpdateRoleRequest(roleRequest)
    }, []);

    useEffect(() => {
        async function checkPermissions() {
            const canApproveAction = await hasRequiredPermissions([MapPermission(AdminPortalPermission.PermissionsRolesApproveEdit)]);
    ...
    async function handleApprove() {
        ...
        let response = await updateRole(updateRoleRequest);
        if (response.statusCode === 200) {
            ...
            if (upsertResponse.statusCode === 200) {
                const auditRequest: AuditRecordRequest = {
                    ...
                    dataAfter: stagingRecord.dataAfter,
                    dataBefore: stagingRecord.dataBefore,
                    description: `approved update of role ${updateRoleRequest.name}`,
                }
    ...
    async function handleDecline() {
        ...
        if (upsertResponse.statusCode === 200) {
            ...
            const auditRequest: AuditRecordRequest = {
                ...
                dataAfter: JSON.stringify({}),
                dataBefore: JSON.stringify(stagingRecord.dataBefore),
                description: `declined update of role ${updateRoleRequest.name}`,
            }
            await addAuditRecord(auditRequest);
        ...
        <>
            ...
            >
                <ModalContent>
                    {(onClose) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1">Approve Update Role</ModalHeader>

                            {updateRoleRequest && (<ModalBody>
                                    ...
                                    <div className="grid gap-6 mb-1 md:grid-cols-2">
                                        <div>
                                            ...
                                            <input type="text" id="name"
                                                   defaultValue={updateRoleRequest.name}
                                        ...
                                        <div>
                                            ...
                                            <input type="text" id="description"
                                                   defaultValue={updateRoleRequest.description}
                                                   className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
                                            focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400
                                            dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
